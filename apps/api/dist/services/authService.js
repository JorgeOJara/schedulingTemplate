...
import { signAccessToken, signInviteToken, signPasswordResetToken, signRefreshToken, verifyAccessToken, verifyInviteToken, verifyPasswordResetToken, verifyRefreshToken, } from '../utils/authTokens.js';
...
export const validateOwnerRegistration = (data) => {
    const schema = z.object({
        email: emailSchema,
        firstName: nameSchema,
        lastName: nameSchema,
...
export const validateEmployeeJoinRegistration = (data) => {
    const schema = z.object({
        email: emailSchema,
        firstName: nameSchema,
        lastName: nameSchema,
...
export const validateAcceptInvite = (data) => {
...
export const createTokens = (userId, orgId, role) => {
...
export const AuthService = {
    ...
    async registerEmployeeJoin(data) {
        ...
        await Promise.all(admins.map((admin) => NotificationService.createNotification(org.id, admin.id, 'New Employee Join Request', `${firstName} ${lastName} requested to join ${org.name}.`, 'JOIN_REQUEST', pendingUser.id, 'USER')));
        return { pendingUserId: pendingUser.id, orgId: org.id };
    },
    async getPendingEmployees(orgId) {
    ...
    async reviewPendingEmployee(orgId, adminUserId, employeeUserId, decision) {
        ...
        if (decision === 'APPROVE') {
            ...
            await NotificationService.createNotification(orgId, approvedUser.id, 'Join Request Approved', 'Your account is approved. You can now log in and view your schedule.', 'JOIN_REQUEST_APPROVED', approvedUser.id, 'USER');
            return { status: 'APPROVED', user: approvedUser };
        }
        await prisma.user.delete({
            where: { id: pendingUser.id },
        });
        await NotificationService.createNotification(orgId, adminUserId, 'Join Request Rejected', `Join request from ${pendingUser.firstName} ${pendingUser.lastName} was rejected.`, 'JOIN_REQUEST_REJECTED', pendingUser.id, 'USER');
    ...
    async acceptInvite(rawToken, rawPassword) {
    ...
    async resetPassword(rawToken, rawPassword) {
