...
const asHours = (minutes) => Math.round((minutes / 60) * 100) / 100;
const calculateWorkedMinutes = (clockInAt, clockOutAt) => Math.max(Math.floor((clockOutAt.getTime() - clockInAt.getTime()) / (1000 * 60)), 0);
...
const getClockEligibleWindow = (shiftStart, shiftEnd, allowanceMinutes) => {
...
const findBestEligibleShift = (shifts, now, allowanceMinutes) => {
    if (shifts.length === 0) {
...
export const TimeClockService = {
    async getClockStatus(orgId, userId) {
    ...
    async clockIn(orgId, userId, shiftId, force = false) {
        const now = DateTime.now();
        ...
        const allowance = org.clockInEarlyAllowanceMinutes || 5;
        const { earliestClockIn, latestClockIn } = getClockEligibleWindow(shift.startTime, shift.endTime, allowance);
        if (now < earliestClockIn) {
            throw new Error(`Too early to clock in. Earliest allowed clock-in is ${earliestClockIn.toFormat('h:mm a')}`);
        }
        ...
        if (isLate) {
            ...
            const employeeName = employee
                ...
                : 'An employee';
            await Promise.all(managers.map((manager) => NotificationService.createNotification(orgId, manager.id, `Late clock-in (${lateByMinutes}m)`, `${employeeName} clocked in ${lateByMinutes} minute(s) late.`, 'WARNING', entry.id, 'TIME_ENTRY')));
        }
        return entry;
    },
    async clockOut(orgId, userId, payload) {
        const now = new Date();
        let entry = null;
        ...
        const needsOvertimeApproval = Boolean(entry.scheduledEnd) && now.getTime() > entry.scheduledEnd.getTime();
        ...
        if (needsOvertimeApproval) {
            ...
            await Promise.all(managers.map((manager) => NotificationService.createNotification(orgId, manager.id, 'Overtime Approval Needed', `${employeeName} clocked out after scheduled end and needs approval.`, 'OVERTIME_APPROVAL', updated.id, 'TIME_ENTRY')));
        }
        return {
            ...updated,
            workedMinutes,
    ...
    async getMyWeeklyHours(orgId, userId, weekStartIso) {
        const now = new Date();
        ...
        const actualMinutes = entries.reduce((acc, entry) => {
            if (!entry.clockOutAt)
                return acc;
            const effectiveClockOutAt = entry.status === 'PENDING_OVERTIME_APPROVAL' && entry.scheduledEnd
                ? entry.scheduledEnd
                : entry.clockOutAt;
    ...
    async getOrgWeeklyHoursComparison(orgId, weekId) {
        let week = null;
        if (weekId) {
        ...
        for (const shift of shifts) {
            if (!shift.employeeId || !shift.employee)
                continue;
            const minutes = Math.max(Math.floor((shift.endTime.getTime() - shift.startTime.getTime()) / (1000 * 60)) -
        ...
        for (const entry of entries) {
            if (!entry.employee)
                continue;
            const effectiveClockOutAt = entry.clockOutAt && entry.status === 'PENDING_OVERTIME_APPROVAL' && entry.scheduledEnd
                ? entry.scheduledEnd
                : entry.clockOutAt;
        ...
        const employees = [...allEmployeeIds].map((employeeId) => {
            ...
            return {
                ...
                actualHours: asHours(actualMinutes),
                differenceHours: asHours(actualMinutes - scheduledMinutes),
                completionRatePct: scheduledMinutes > 0
        ...
        const totalScheduledMinutes = employees.reduce((acc, employee) => acc + Math.round(employee.scheduledHours * 60), 0);
        const totalActualMinutes = employees.reduce((acc, employee) => acc + Math.round(employee.actualHours * 60), 0);
        return {
            weekId: week.id,
    ...
    async getPendingOvertimeRequests(orgId) {
    ...
    async reviewOvertimeRequest(orgId, _approverId, timeEntryId, decision) {
        ...
        await NotificationService.createNotification(orgId, entry.employeeId, decision === 'APPROVE' ? 'Overtime Approved' : 'Overtime Denied', decision === 'APPROVE'
            ? 'Your late clock-out time was approved.'
            : 'Your late clock-out time was denied and adjusted to scheduled end time.', decision === 'APPROVE' ? 'OVERTIME_APPROVED' : 'OVERTIME_DENIED', timeEntryId, 'TIME_ENTRY');
        return updated;
    },
};
