...
export const ShiftService = {
    async createShift(orgId, data) {
        const { scheduleWeekId, employeeId, departmentId, locationId, shiftGroup, startTime, endTime, breakDurationMinutes = 0, breakIsPaid = false, notes, } = data;
        // Validate org exists
    ...
    async updateShift(shiftId, orgId, data) {
        // Get current shift
    ...
    async deleteShift(shiftId, orgId) {
    ...
    async getShiftsByWeek(weekId, orgId, filters) {
        const where = {
    ...
    async getShiftsByEmployee(orgId, userId, startDate, endDate) {
        return await prisma.shift.findMany({
            where: {
                employeeId: userId,
                scheduleWeek: { orgId },
    ...
    calculateHours(shifts, orgTimezone) {
        let totalSeconds = 0;
        let breakMinutes = 0;
    ...
    async checkShiftConflicts(weekId, employeeId, startTime, endTime, excludeShiftId) {
        const where = {
            scheduleWeekId: weekId,
            employeeId,
            OR: [
