...
export const ShiftSwapService = {
    async createSwap(data) {
        ...
        await NotificationService.createNotification(orgId, responderId, 'Shift Swap Requested', `${requestor.firstName} ${requestor.lastName} requested to swap shifts with you.`, 'SHIFT_SWAP_REQUEST', swapRequest.id, 'SHIFT_SWAP_REQUEST');
        return swapRequest;
    },
    async updateSwap(swapId, orgId, data) {
        ...
        if (status === 'APPROVED') {
            await AuditLogService.createAuditLog(orgId, approvedById, 'SWAP_APPROVED', swapId, 'ShiftSwapRequest', { status: oldStatus }, updatedSwap);
        }
        else if (status === 'DENIED') {
            await AuditLogService.createAuditLog(orgId, approvedById, 'SWAP_REJECTED', swapId, 'ShiftSwapRequest', { status: oldStatus }, updatedSwap);
        }
    ...
    async acceptSwap(swapId, responderId, orgId) {
        const swap = await prisma.shiftSwapRequest.findFirst({
        ...
        await Promise.all([
            NotificationService.createNotification(orgId, swap.requestorId, 'Shift Swap Accepted by Coworker', 'Your coworker accepted the swap. It is now awaiting manager approval.', 'SHIFT_SWAP_ACCEPTED', swapId, 'SHIFT_SWAP_REQUEST'),
            ...reviewers.map((reviewer) => NotificationService.createNotification(orgId, reviewer.id, 'Shift Swap Awaiting Approval', `${swap.requestor.firstName} has a shift swap ready for manager review.`, 'SHIFT_SWAP_REVIEW', swapId, 'SHIFT_SWAP_REQUEST')),
        ]);
        return updatedSwap;
    },
    async denySwap(swapId, responderId, orgId) {
        const swap = await prisma.shiftSwapRequest.findFirst({
            where: { id: swapId, responderId, orgId },
        });
        if (!swap) {
        ...
        await NotificationService.createNotification(orgId, swap.requestorId, 'Shift Swap Declined', 'Your coworker declined this shift swap request.', 'SHIFT_SWAP_DECLINED', swapId, 'SHIFT_SWAP_REQUEST');
        return updatedSwap;
    },
    async getRequest(swapId, orgId) {
        const swap = await prisma.shiftSwapRequest.findFirst({
            where: { id: swapId, orgId },
            include: {
    ...
    async getOrgSwaps(orgId, filters) {
        const where = { orgId };
        if (filters?.status)
