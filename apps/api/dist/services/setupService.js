...
const isTemplateOnOpenDay = (template, businessHoursByDay) => {
    const dayHours = businessHoursByDay.get(template.dayOfWeek);
    return Boolean(dayHours && !dayHours.isClosed);
};
const buildHoursMapByLocation = (locationIds, payload) => {
...
export const SetupService = {
    ...
    async completeOrgSetup(orgId, data) {
        const normalizedTemplates = normalizeTemplates(data.defaultShiftTemplates ?? []);
        if (data.clockInEarlyAllowanceMinutes !== undefined &&
        ...
        normalizedTemplates.forEach((template, index) => {
            ...
            const hoursByDay = new Map((locationHours ?? []).map((entry) => [entry.dayOfWeek, entry]));
            if (templateLocationId && !isTemplateOnOpenDay(template, hoursByDay)) {
                throw new Error(`Default shift template #${index + 1} is on a closed day for its location`);
            }
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const hoursRows = Array.from(hoursByLocation.entries()).flatMap(([locationId, hours]) => hours.map((entry) => ({
                orgId,
                locationId,
                dayOfWeek: entry.dayOfWeek,
    ...
    async applyDefaultTemplatesToWeek(orgId, weekId) {
        ...
        for (const template of templates) {
            ...
            const start = DateTime.fromFormat(`${dayLabel} ${template.startTime}`, 'yyyy-LL-dd HH:mm', { zone: org.timezone });
            let end = DateTime.fromFormat(`${dayLabel} ${template.endTime}`, 'yyyy-LL-dd HH:mm', { zone: org.timezone });
            if (end <= start) {
                end = end.plus({ days: 1 });
            }
