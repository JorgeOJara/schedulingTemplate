...
export const LocationService = {
    async createLocation(orgId, name, address, phone) {
    ...
    async updateLocation(orgId, id, name, address, phone, active) {
    ...
    async getLocation(orgId, id) {
        return await prisma.location.findFirst({
            where: { id, orgId },
            include: { shifts: true, defaultShiftTemplates: true },
        });
    },
    async getLocations(orgId, activeOnly = true) {
        return await prisma.location.findMany({
            where: { orgId, active: activeOnly },
        });
    },
    async deleteLocation(orgId, id) {
        const location = await prisma.location.findFirst({ where: { id, orgId } });
        if (!location) {
            throw new Error('Location not found');
        }
