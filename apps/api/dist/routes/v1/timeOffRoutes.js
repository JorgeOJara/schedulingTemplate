...
router.put('/:id/approve', async (req, res) => {
    try {
        ...
        const requestId = String(req.params.id);
        const request = await TimeOffService.updateRequest(requestId, req.user.orgId, { status: 'APPROVED', approvedById: req.user.id, notes: req.body.notes });
        res.status(200).json(request);
    }
    catch (error) {
...
router.put('/:id/deny', async (req, res) => {
    try {
        ...
        const requestId = String(req.params.id);
        const request = await TimeOffService.updateRequest(requestId, req.user.orgId, { status: 'DENIED', approvedById: req.user.id, notes: req.body.notes });
        res.status(200).json(request);
    }
    catch (error) {
