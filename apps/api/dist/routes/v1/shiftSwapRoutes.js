...
router.put('/:id/accept', async (req, res) => {
    try {
        const swapId = String(req.params.id);
        const swap = await ShiftSwapService.acceptSwap(swapId, req.user.id, req.user.orgId);
        res.status(200).json(swap);
    }
    catch (error) {
...
router.put('/:id/deny', async (req, res) => {
    try {
        const swapId = String(req.params.id);
        const swap = await ShiftSwapService.denySwap(swapId, req.user.id, req.user.orgId);
        res.status(200).json(swap);
    }
    catch (error) {
...
router.put('/:id/approve', async (req, res) => {
    try {
        ...
        const swapId = String(req.params.id);
        const swap = await ShiftSwapService.updateSwap(swapId, req.user.orgId, { status: 'APPROVED', approvedById: req.user.id, notes: req.body.notes });
        res.status(200).json(swap);
    }
    catch (error) {
...
router.put('/:id/manager-deny', async (req, res) => {
    try {
        if (!['ADMIN', 'MANAGER'].includes(req.user.role)) {
            return res.status(403).json({ error: 'Only managers can deny shift swaps' });
        }
        const swapId = String(req.params.id);
        const swap = await ShiftSwapService.updateSwap(swapId, req.user.orgId, { status: 'DENIED', approvedById: req.user.id, notes: req.body.notes });
        res.status(200).json(swap);
    }
    catch (error) {
