...
const setupPayloadSchema = z.object({
    ...
    locationBusinessHours: z
        ...
        .optional(),
    defaultShiftTemplates: z.array(z.object({
...
router.get('/org', rbacMiddleware(['ADMIN', 'MANAGER']), async (req, res) => {
    try {
...
router.put('/org', rbacMiddleware(['ADMIN', 'MANAGER']), async (req, res) => {
    try {
        const payload = setupPayloadSchema.parse(req.body);
...
router.post('/week/:weekId/apply-defaults', rbacMiddleware(['ADMIN', 'MANAGER']), async (req, res) => {
    try {
