...
const clockOutSchema = z.object({
    ...
    shiftId: z.string().uuid().optional(),
}).refine((value) => !(value.timeEntryId && value.shiftId), { message: 'Provide either timeEntryId or shiftId, not both' });
...
router.get('/my-week', async (req, res) => {
    try {
        ...
        const report = await TimeClockService.getMyWeeklyHours(req.user.orgId, req.user.id, query.weekStart);
...
router.put('/overtime-requests/:id/approve', async (req, res) => {
    try {
        if (!['ADMIN', 'MANAGER'].includes(req.user.role)) {
            return res.status(403).json({ error: 'Only managers can approve overtime requests' });
        }
        const updated = await TimeClockService.reviewOvertimeRequest(req.user.orgId, req.user.id, String(req.params.id), 'APPROVE');
...
router.put('/overtime-requests/:id/deny', async (req, res) => {
    try {
        if (!['ADMIN', 'MANAGER'].includes(req.user.role)) {
            return res.status(403).json({ error: 'Only managers can deny overtime requests' });
        }
        const updated = await TimeClockService.reviewOvertimeRequest(req.user.orgId, req.user.id, String(req.params.id), 'DENY');
        res.status(200).json(updated);
    }
    catch (error) {
