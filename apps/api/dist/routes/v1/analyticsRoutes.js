...
router.get('/weekly-summary', rbacMiddleware(['ADMIN', 'MANAGER']), async (req, res) => {
    try {
...
router.get('/monthly-report', rbacMiddleware(['ADMIN', 'MANAGER']), async (req, res) => {
    try {
        const locationId = req.query.locationId ? z.string().uuid().parse(String(req.query.locationId)) : undefined;
...
router.get('/department-breakdown', rbacMiddleware(['ADMIN', 'MANAGER']), async (req, res) => {
    try {
...
router.get('/coverage-metrics', rbacMiddleware(['ADMIN', 'MANAGER']), async (req, res) => {
    try {
        const weekId = req.query.weekId ? z.string().uuid().parse(String(req.query.weekId)) : undefined;
...
router.get('/stats', rbacMiddleware(['ADMIN', 'MANAGER']), async (req, res) => {
    try {
...
router.get('/hours-comparison', rbacMiddleware(['ADMIN', 'MANAGER']), async (req, res) => {
    try {
        const weekId = req.query.weekId ? z.string().uuid().parse(String(req.query.weekId)) : undefined;
        const exportFormat = req.query.export ? String(req.query.export) : undefined;
        const report = await TimeClockService.getOrgWeeklyHoursComparison(req.user.orgId, weekId);
        if (exportFormat === 'csv') {
            const headers = [
                'Employee Name',
                'Email',
            ...
            res.setHeader('Content-Disposition', `attachment; filename="hours-comparison-${report.weekStart.toISOString().slice(0, 10)}.csv"`);
